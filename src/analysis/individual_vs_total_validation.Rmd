...
3. Check if trivial variable like program length doesn't provide a good-enough estimate for cost
...
boxplot(measure_total_time_ns ~ run_id, data=total_measurements, las=2)
```
...
{
  program_measurements = total_measurements[which(total_measurements$program_id==program_id), ]
  plot(program_measurements$measure_total_time_ns)
}
```
...
par(mfrow=c(1,3))
plot_validation(validation, explained_variable, 'program_length')
plot_validation(validation, explained_variable, 'n_push')
plot_validation(validation, explained_variable, 'n_pop ')
```
