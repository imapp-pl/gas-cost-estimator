...
estimated_programs = sqldf("SELECT
                           ...
                           LEFT JOIN marginal_estimated_cost ON ops_per_program.op == marginal_estimated_cost.op 
...
scatter.smooth(estimate_comparison$estimate_marginal_ns ~ estimate_comparison$min)
...
#points(estimate_comparison$estimate_marginal_ns)
#text(estimate_comparison$estimate_marginal_ns, labels=paste(estimate_comparison$op, estimate_comparison$env), cex=0.6, pos=1, font=2)
```
...
1. Slight non-linearity for `geth` (costlier programs overestimated)
