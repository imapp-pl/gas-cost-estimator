...
plot_validation <- function(df, yvar, xvar, label_n_zero_args) {
  ...
  scatter.smooth(df[,xvar], df[,yvar], col=df$has_exp, asp=1)
...
```
python3 program_generator/pg_arythmetic.py generate --count=100 --opsLimit=30 --cleanStack --push=1 --fullCsv > ../../local/pg_arythmetic_c100_ops30.csv`.
```
...
```
docker run \
  --rm -v ~/sources/imapp/local:/srv/local \
  -it gas-cost-estimator/geth_all \
  sh -c "cd src && cat <program set filename> | python3 instrumentation_measurement/measurements.py measure --evm geth --mode trace --sampleSize 1 > /srv/local/trace_pg_arythmetic_c100_ops30.csv"
```
...
docker run \
  ...
  -v ~/sources/imapp/local:/srv/local \
...
warmup_profile_plots(measurements, 'env', 'geth')
warmup_profile_plots(measurements, 'env', 'evmone')
warmup_profile_plots(measurements, 'env', 'openethereum')
