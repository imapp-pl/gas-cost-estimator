...
```{r}
plot_validation <- function(df, yvar, xvar, labels, colors, set_asp) {
  if (missing(labels)) {
    labels = 'program_id'
  }
  if (missing(colors)) {
  ...
  scatter.smooth(df[, xvar], df[, yvar], col=df[, colors], asp=asp)
...
program_set_codename = "full5_c100_randOps1000_clean_prejump"
measurement_codename = "200_8"
...
# total_measurements_openethereum = read.csv(paste("../../local/openethereum_pg_validation_", program_set_codename, "_", "16_400", ".csv", sep=""))
...
fingerprint <- function(x) {
  excluded = x$op %in% c('PUSH4', 'PUSH1', 'MSTORE8', 'STOP', 'POP')
  opcodes = unique(x[which(!excluded),'op'])
  paste(opcodes, collapse=' ')
}
fingerprint_no_push <- function(x) {
  excluded = x$op %in% c('PUSH1', 'PUSH2', 'PUSH3', 'PUSH4', 'PUSH5', 'PUSH6', 'PUSH7', 'PUSH8', 'PUSH9', 'PUSH10', 'PUSH11', 'PUSH12', 'PUSH13', 'PUSH14', 'PUSH15', 'PUSH16', 'PUSH17', 'PUSH18', 'PUSH19', 'PUSH20', 'PUSH21', 'PUSH22', 'PUSH23', 'PUSH24', 'PUSH25', 'PUSH26', 'PUSH27', 'PUSH28', 'PUSH29', 'PUSH30', 'PUSH31', 'PUSH32', 'MSTORE8', 'STOP', 'POP')
  opcodes = unique(x[which(!excluded),'op'])
  paste(opcodes, collapse=' ')
}
has_exp <- function(x) {
...
```{r}
program_length_model_geth = lm_validation(validation, explained_variable, 'program_length', selection_col='env', selection_val='geth')
program_length_model_evmone = lm_validation(validation, explained_variable, 'program_length', selection_col='env', selection_val='evmone')
# program_length_model_openethereum = lm_validation(validation, explained_variable, 'program_length', selection_col='env', selection_val='openethereum')
print('geth all - only progam length')
summary(program_length_model_geth)
print('evmone all - only progam length')
summary(program_length_model_evmone)
# print('openethereum all - only progam length')
# summary(program_length_model_openethereum)
```
...
```{r}
compare_plots <- function(df, selection_col, selection_val, estimate_var, labels, colors) {
  if (missing(labels)) {
    labels = 'program_id'
  }
  if (missing(colors)) {
  ...
  plot_validation(subset, explained_variable, estimate_var, labels=labels, colors=colors) #, set_asp=TRUE)
...
compare_plots(validation, 'env', 'geth', 'cost_ns', labels='program_length')
compare_plots(validation, 'env', 'evmone', 'cost_ns', labels='program_length')
