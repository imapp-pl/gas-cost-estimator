...
arg0_estimated_cost_lin = estimated_traces$arg0_ns * estimated_traces$arg0 / 32 * (estimated_traces$op %in% arg0_linear_opcodes)
arg1_estimated_cost_lin = estimated_traces$arg1_ns * estimated_traces$arg1 / 32 * (estimated_traces$op %in% arg1_linear_opcodes)
arg2_estimated_cost_lin = estimated_traces$arg2_ns * estimated_traces$arg2 / 32 * (estimated_traces$op %in% arg2_linear_opcodes)
...
get_gas_schedule_comparison <- function(pivot_opcode, df) {
  ...
  # 
  for (env in all_envs) {
    df$estimate_marginal_ns_rel[which(df$env==env)] = df$estimate_marginal_ns[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$arg0_ns_rel[which(df$env==env)] = df$arg0_ns[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$arg1_ns_rel[which(df$env==env)] = df$arg1_ns[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$arg2_ns_rel[which(df$env==env)] = df$arg2_ns[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$expensive_ns_rel[which(df$env==env)] = df$expensive_ns[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$estimate_marginal_gas[which(df$env==env)] = df$estimate_marginal_ns_rel[which(df$env == env)] * pivot_gas_cost
    df$arg0_gas[which(df$env==env)] = df$arg0_ns_rel[which(df$env == env)] * pivot_gas_cost
    df$arg1_gas[which(df$env==env)] = df$arg1_ns_rel[which(df$env == env)] * pivot_gas_cost
    df$arg2_gas[which(df$env==env)] = df$arg2_ns_rel[which(df$env == env)] * pivot_gas_cost
    df$expensive_gas[which(df$env==env)] = df$expensive_ns_rel[which(df$env == env)] * pivot_gas_cost
    df$estimate_marginal_ns_rel_stderr[which(df$env==env)] = df$estimate_marginal_ns_stderr[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$arg0_ns_rel_stderr[which(df$env==env)] = df$arg0_ns_stderr[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$arg1_ns_rel_stderr[which(df$env==env)] = df$arg1_ns_stderr[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$arg2_ns_rel_stderr[which(df$env==env)] = df$arg2_ns_stderr[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$expensive_ns_rel_stderr[which(df$env==env)] = df$expensive_ns_stderr[which(df$env == env)] / pivot_statistics[env, 'estimate_marginal_ns']
    df$estimate_marginal_gas_stderr[which(df$env==env)] = df$estimate_marginal_ns_rel_stderr[which(df$env == env)] * pivot_gas_cost
    df$arg0_gas_stderr[which(df$env==env)] = df$arg0_ns_rel_stderr[which(df$env == env)] * pivot_gas_cost
    df$arg1_gas_stderr[which(df$env==env)] = df$arg1_ns_rel_stderr[which(df$env == env)] * pivot_gas_cost
    df$arg2_gas_stderr[which(df$env==env)] = df$arg2_ns_rel_stderr[which(df$env == env)] * pivot_gas_cost
    df$expensive_gas_stderr[which(df$env==env)] = df$expensive_ns_rel_stderr[which(df$env == env)] * pivot_gas_cost
  }
  ...
  return(list("gas_schedule_comparison" = gas_schedule_comparison, "estimated_cost" = df))
}


pivot_results = data.frame(matrix(ncol=2, nrow=0))
colnames(pivot_results) = c('pivot_opcode', 'distance')
for (pivot_opcode in all_opcodes) {
  gas_schedule_comparison = get_gas_schedule_comparison(pivot_opcode, estimated_cost)$gas_schedule_comparison
  distance = sqrt(sum((gas_schedule_comparison$geth_gas - gas_schedule_comparison$evmone_gas)^2))
  pivot_results[nrow(pivot_results) + 1, ] = data.frame(pivot_opcode, distance)
}
```
...
gas_schedule_comparison = get_gas_schedule_comparison(pivot_opcode, estimated_cost)$gas_schedule_comparison
estimated_cost = get_gas_schedule_comparison(pivot_opcode, estimated_cost)$estimated_cost
...
gas_schedule_comparison[nrow(gas_schedule_comparison) + 1, ] =
  data.frame("DIV_expensive_cost", 0, estimated_cost$expensive_gas[which(estimated_cost$env=='geth' & estimated_cost$opcode=='DIV')],  estimated_cost$expensive_gas[which(estimated_cost$env=='evmone' & estimated_cost$opcode=='DIV')])
gas_schedule_comparison[nrow(gas_schedule_comparison) + 1, ] =
  data.frame("SDIV_expensive_cost", 0, estimated_cost$expensive_gas[which(estimated_cost$env=='geth' & estimated_cost$opcode=='SDIV')],  estimated_cost$expensive_gas[which(estimated_cost$env=='evmone' & estimated_cost$opcode=='SDIV')])
gas_schedule_comparison[nrow(gas_schedule_comparison) + 1, ] =
  data.frame("MOD_expensive_cost", 0, estimated_cost$expensive_gas[which(estimated_cost$env=='geth' & estimated_cost$opcode=='MOD')],  estimated_cost$expensive_gas[which(estimated_cost$env=='evmone' & estimated_cost$opcode=='MOD')])
gas_schedule_comparison[nrow(gas_schedule_comparison) + 1, ] =
  data.frame("SMOD_expensive_cost", 0, estimated_cost$expensive_gas[which(estimated_cost$env=='geth' & estimated_cost$opcode=='SMOD')],  estimated_cost$expensive_gas[which(estimated_cost$env=='evmone' & estimated_cost$opcode=='SMOD')])
gas_schedule_comparison[nrow(gas_schedule_comparison) + 1, ] =
  data.frame("ADDMOD_expensive_cost", 0, estimated_cost$expensive_gas[which(estimated_cost$env=='geth' & estimated_cost$opcode=='ADDMOD')],  estimated_cost$expensive_gas[which(estimated_cost$env=='evmone' & estimated_cost$opcode=='ADDMOD')])
gas_schedule_comparison[nrow(gas_schedule_comparison) + 1, ] =
  data.frame("MULMOD_expensive_cost", 0, estimated_cost$expensive_gas[which(estimated_cost$env=='geth' & estimated_cost$opcode=='MULMOD')],  estimated_cost$expensive_gas[which(estimated_cost$env=='evmone' & estimated_cost$opcode=='MULMOD')])
