...
plot_validation <- function(df, yvar, xvar, labels, colors, set_asp, log) {
  ...
  plot(df[, xvar], df[, yvar], col=df[, colors], asp=asp, log=log)
  abline(lm(df[, yvar] ~ df[, xvar]), asp=asp, untf=TRUE)
...
compare_plots <- function(df, selection_col, selection_val, estimate_var, labels, colors, log) {
  ...
  title(main=paste(selection_val, 'estimate by', estimate_var), cex.main=3)
}
...
The model diagnosticts aren't ideal, especially for `geth`, where some non-linearity is present..
