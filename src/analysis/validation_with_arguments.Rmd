...
program_set_codename = "full5_c100_randOps1000_clean_prejump_randominant"
measurement_codename = "200_8"
...
```{r fig.width=15}
par(mfrow=c(1,2))
trivial_variable_plots <- function(df, selection_col, selection_val, labels, colors) {
  if (missing(labels)) {
    labels = 'program_id'
  }
  if (missing(colors)) {
  ...
  subset = df[which(df[, selection_col] == selection_val), ]
  plot_validation(subset, explained_variable, 'program_length', labels=labels, colors=colors)
